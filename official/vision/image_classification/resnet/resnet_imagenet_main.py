...
def run():
    ...
    single_l2_loss_op = False #'Calculate L2_loss on concatenated weights, instead of using Keras per-layer L2 loss.'
    ...
    per_epoch_steps, train_epochs, eval_steps = get_num_train_iterations_non_flag(batch_size, train_epochs,
                                                                                  num_images_train,
                                                                                  num_images_val,
    ...
    with distribution_utils.get_strategy_scope(strategy):
        runnable = resnet_hops_runnable.ResnetHopsRunnable(
            use_tf_while_loop, use_tf_function, dtype, global_batch_size, datasets_num_private_threads,
            use_synthetic_data, single_l2_loss_op, loss_scale, fp16_implementation, data_dir, num_images_train,
            time_callback, epoch_steps)
...
def main(_):
    ...
    stats = run() #debugflags(flags.FLAGS)
